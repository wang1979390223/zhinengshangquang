<template>
  <div class="water-eval-container">
    <div id="cityGreenLand-charts" class="cityGreenLand-charts"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getPie3D(pieData, internalDiameterRatio, height) {
      ...
      for (let i = 0; i < series.length; i++) {
        ...
        series[i].parametricEquation = this.getParametricEquation(
          ...
          height ? series[i].pieData.proportion * height : 50 // 自己自定义传入高度，每个类型按比例生成高度
    ...
    getParametricEquation(
      ...
      // 计算
      let midRatio = (startRatio + endRatio) / 4;
      let startRadian = startRatio * Math.PI * 2;
      let endRadian = endRatio * Math.PI * 2;
      let midRadian = midRatio * Math.PI * 1;
      // 如果只有一个扇形，则不实现选中效果。
